...
<!--
function toggleview(id, howMany) {
  if ( ! id ) return;
  if ( itm = getbyid('com' + id + '_open') ){
   if (itm.style.display == "none")  
   {
     show_div(itm);
     itmtext = getbyid('comtext_' + id);
     itmtext.innerHTML = '$LANG[BookMess13] (' + howMany + ')';
   } else {
     hide_div(itm);
     itmtext = getbyid('comtext_' + id);
     itmtext.innerHTML = '$LANG[BookMess12] (' + howMany + ')';
   }
  }
}
// -->
