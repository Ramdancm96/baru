 ...
 * Last Modified: Tue, 11 October 2011 20:06:33 GMT
 * ----------------------------------------------
 */
...
//
  ...
  function check_comment()
  {
    ...
    {
      return $this->comment_form($this->db->gb_error($this->db->LANG['ErrorPost4'].' (4)'));
    }
    if (($this->db->VARS['need_pass'] == 1) && empty($this->pass_comment)) 
    {
      return $this->comment_form($this->db->gb_error($this->db->LANG['ErrorPost13']));
    }
    ...
    {
      if(($failedHeader = $this->db->check_headers(2)) != 0)
      {
        return $this->comment_form($this->db->gb_error($this->db->LANG['ErrorPost4'].' (5.'.$failedHeader.')'));
      }
    }    
    ...
    if ($this->db->VARS['need_pass'] == 1)
    {
      ...
      {
        return $this->comment_form($this->db->gb_error($this->db->LANG['PassMess3']));
      }
    }
    if ($this->db->VARS['need_pass'] == 2)
    {
      if (($this->db->VARS['solve_media'] == 0) && (!$this->db->captcha_test($this->bottest, $this->timehash)))
      {
        return $this->comment_form($this->db->gb_error($this->db->LANG['ErrorPost14']));
      }
      elseif ($this->db->VARS['solve_media'] == 1) 
      {
        ...
        {
        	return $this->comment_form($this->db->gb_error($this->db->LANG['ErrorPost14']),0,1);
        }
      }      
    }
    ...
    if ($this->db->VARS['banned_ip'] == 1)
    {
      ...
      {
        return $this->comment_form($this->db->gb_error($this->db->LANG['ErrorPost9']));
      }
    }
...
//
  ...
  function insert_comment()
  {
    ...
    $this->db->query("INSERT INTO ".LAZ_TABLE_PREFIX."_com (id,name,email,comments,host,timestamp,comaccepted,ip) VALUES ('$this->id','$this->user','$this->email','".mysql_real_escape_string($this->comment)."','$host','$the_time','$this->accepted','$this->ip')");
    ...
    $messagebody .= (!empty($this->email)) ? $LANG['FormEmail'] . ': ' . $this->email : '';
