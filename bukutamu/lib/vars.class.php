 ...
 * Last Modified: Mon, 19 December 2011 11:57:18 GMT
 * ----------------------------------------------
 */
...
// Have we banned the ip they are posting from?
  ...
  function isBannedIp($ip)
  {
    ...
    {
      if (strpos($row['ban_ip'], '/') !== false) // IP check using CIDR
      {
        list ($startIP, $rPrefix) = split('/', $row['ban_ip']);
...
// Something is wrong so lets report it
  ...
  function gb_error($ERROR)
  {
    ...
    {
      $this->query('UPDATE ' . LAZ_TABLE_PREFIX . '_config SET block_count=block_count + 1 WHERE config_id=' . $this->VARS['config_id']);
    }
    $LANG =& $this->LANG;
    $VARS =& $this->VARS;
    $EMAILJS = '';
...
// Lets show a nice picture of what browser they posted with
  ...
  {
    $browsers = array('opera', 'flock', 'firefox', 'chrome', 'iphone', 'android', 'safari', 'konqueror', 'netscape', 'aol', 'camino', 'chimera', 'crazy', 'galeon', 'k-meleon', 'maxthon', 'slimbrowser', 'amaya', 'avantbrowser', 'msie', 'gecko', 'mozilla');
