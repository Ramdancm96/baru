...
class admin {
	...
	function index() {
		...
		section_content('
			<IFRAME SRC="http://www.jcow.net/news.php?s='.urlencode($s).'&e='.urlencode($client['email']).'fn='.urlencode($client['fullname']).'&m='.$member['num'].'&v='.jversion().'" TITLE="Jcow News" WIDTH="580" HEIGHT="180" scrolling="no">
<a href="http://www.jcow.net">Visit Jcow for News&Updates</a><br />
		...
		if ($members > 100) {
			$rclosed = '<span style="color:red;font-weight:bold;">Registration Closed</span>';
		}
		
		
		section_content('
		<ul>
		<li>'.url('admin/pending_posts','<strong>'.$postqueue.'</strong> post(s) in Queue').' | '.url('admin/memberqueue','<strong>'.$memberqueue.'</strong> member(s) in Queue').'</li>
		...
		<li>Your Jcow Version: <strong>'.jversion().'</strong> Community Edition<br />
		In Community Edition, you have a <strong>100-members</strong> Limit. '.$rclosed.' [<a href="http://www.jcow.net/download/" target="_blank"> <strong>Upgrade</strong> to Pro</a>]</li>
		</ul>');
		section_content('<div>
		...
		<div class="ai_items">
		<img src="'.uhome().'/files/icons/admin/permissions.gif" />
		<br /><span class="sub">available in Pro</span></div>

		<div class="ai_items">
		<img src="'.uhome().'/modules/admin/member_quick_management.gif" />
		<br /><span class="sub">available in Pro</span></div>

		<div class="ai_items">
		<img src="'.uhome().'/modules/admin/stream_monitor.gif" />
		<br /><span class="sub">available in Pro</span></div>

		<div class="ai_items">
		<img src="'.uhome().'/modules/admin/cache.gif" />
		<br /><span class="sub">available in Pro</span></div>
		...
		c('
		<li>'.url('http://community.jcow.net/extensions', 'Jcow app store').' - Get Jcow extensions</li>
	...
	if (get_gvar('private_network')) {
		...
		<p>
		'.label('Signing Up').'
		<select name="only_invited">
		<option value="0" '.$only_invited_no.' >Public - everyone can sign up to your site</option>
		<option value="1" '.$only_invited.' >Private - only invited people can sign up</option>
		</select>
		</p>
		<p>
		'.label('Limit').'
		...
		<option value="0" '.$acc_verify_0.' >Auto verified (for Dev/Test)</option>
		<option value="2" '.$acc_verify_2.' >Manually verified (Recommended, for Production)</option>
		</select><br />
		Allow <input type="text" size="3" name="pending_post_limit" value="'.$pending_post_limit.'" /> posts before verifed. 
		<span>(Must be greater than 1. The posts will go to '.url('admin/pending_posts','Queue').')</span>
		</p>
		<p>
		'.label('Email Confirmation').'
		<input type="checkbox" value="1" name="email_c" '.$email_c.' /> Enable email confirmation.<br />
		<span>Newcomers must check their email inbox for the "confirmation code" to finish the registration</span>
		</p>
	...
	function configpost() {
		...
		set_gvar('email_c',$_POST['email_c']);
		...
		set_gvar('only_invited',$_POST['only_invited']);
