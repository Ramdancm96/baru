		...
		if (preg_match("/^page_/i",$uri)) {
			...
			elseif ($uri == 'connections') {
				if (!$client['id']) {
					...
					$uids[] = $client['id'];
					$num = 10;
					$res2 = sql_query("select f.fid from ".tb()."friends as f left join ".tb()."accounts as u on u.id=f.fid where f.uid='{$client['id']}' order by u.lastlogin desc limit 10");
					while ($row = sql_fetch_array($res2)) {
						$uids[] = $row['fid'];
					}
					$res2 = sql_query("select f.fid from ".tb()."followers as f left join ".tb()."accounts as u on u.id=f.fid where f.uid='{$client['id']}' order by u.lastlogin desc limit 10");
					while ($row = sql_fetch_array($res2)) {
						$uids[] = $row['fid'];
					}
					$uids = implode(',',$uids);
			...
			elseif (is_numeric($uri)) {
				if($cat = valid_category($uri, $this->name)) {
					...
					nav(h($cat['name']));
					$pwhere = " and cid='$cid' ";
					$res = sql_query("SELECT s.*,u.fullname,u.username FROM `".tb()."stories` as s left join `".tb()."accounts` as u on u.id=s.uid where s.app='{$this->name}' and s.cid='$cid' ORDER by s.id DESC LIMIT $offset,$num_per_page ");
				}
				else {
					die('wrong cid');
				}
				...
				$uri = 'all';
				$res = sql_query("SELECT s.*,u.username FROM `".tb()."stories` as s left join `".tb()."accounts` as u on u.id=s.uid where s.app='{$this->name}' and s.page_type='u' ORDER by s.id DESC LIMIT $offset,$num_per_page ");
			}
...
'.$current_menu_path.'
	...
	<script language="javascript" type="text/javascript">
		...
		language : "en",
		theme_advanced_buttons1 : "code,bold,italic,underline,bullist,undo,redo,link,unlink,forecolor,removeformat,cleanup",
	...
	function show_content($content) {
		return strip_tags($content,'<p><br><b><strong><img><a>');
	}
